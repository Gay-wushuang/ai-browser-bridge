...
驱动提供商真实的网页界面，并只向网页 AI 提供六个经过校验的仓库工具：`list_files`、
`grep_code`、`read_file`、`apply_patch`、`run_tests` 和 `git_diff`。因此你可以继续使用网页订阅中的模型、
...
为减少网页往返，一次回复最多可以批量请求 8 个互相独立的只读工具（目录、搜索、读取、
diff）。补丁和测试仍要求单独请求，确保修改建立在已经看到的结果之上。每次工具结果都会附带
原始任务、已完成工具及剩余轮次的紧凑摘要，避免长任务中途丢失目标。
